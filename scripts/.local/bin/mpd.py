    ...
    def run(self):
        ...
        if len(mpc_status) == 2:
            ...
            if status_test:
                ...
                print_status = "<span foreground='#9AD8E3' font_weight='normal'>{}</span>".format(print_status)

                # output long format
                mpc_status_text = mpc_status[0]
                if self.escape:
                    mpc_status_text = escape(mpc_status_text)
                print("{} {}".format(print_status, mpc_status_text))
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--color", dest="color", default="", help="Color of the returned text. Default: none")
    ...
    parser.add_argument("--escape", dest="escape", default=False, action="store_true", help="Escapes the output (&, < and > characters)")
